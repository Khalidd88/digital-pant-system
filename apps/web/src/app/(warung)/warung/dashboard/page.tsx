...
export default function WarungDashboardPage() {
  ...
  const [warungName, setWarungName] = useState("Warung Bu Tejo");
  ...
  // 1. Ambil Profil Warung, Saldo, dan Log Transaksi Realtime dari Backend
  const fetchWarungData = useCallback(async (currentWarungId: string, silent = false) => {
    ...
    try {
      ...
      if (json.success && json.data) {
        ...
        if (data.name) setWarungName(data.name);

        const list: VerificationItem[] = [];
        let bottlesSum = 0;

        // Map data Scan Log botol yang disetor di warung ini
        ...
        // Map data penarikan saldo warung
  ...
  // Inisialisasi Session Mitra Warung & Polling Otomatis Tiap 4 Detik
  useEffect(() => {
    ...
    fetchWarungData(savedId);

    const interval = setInterval(() => {
      fetchWarungData(savedId, true);
    }, 4000);
  ...
  // 2. Submit Penarikan Saldo Riil Warung ke Backend
  const handleExecuteWithdraw = async () => {
    ...
    if (numericAmount > balance) {
      alert(`Saldo warung tidak mencukupi! Saldo aktif: Rp${balance.toLocaleString("id-ID")}`);
      return;
    }
    ...
    try {
      ...
      if (!res.ok || !json.success) {
        throw new Error(json.message || "Gagal mencairkan saldo warung.");
      }
  ...
  const handleVerifyBottleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Refresh data dashboard warung seketika
      fetchWarungData(warungId, true);
    } catch (err: any) {
        ...
        <div className="block md:hidden w-full sticky top-0 z-30 pt-3 px-3 sm:px-4 backdrop-blur-md">
          <div className="flex w-full items-center justify-between px-4 py-3 bg-[linear-gradient(180deg,#1F6A76_0%,#0B424F_100%)] rounded-[18px] shadow-md border border-[#52C3BF]/20 text-white">
            ...
            <div className="flex items-center gap-2">
              <button 
                type="button"
                onClick={() => fetchWarungData(warungId)}
          ...
          <header className="hidden md:flex w-full h-[84px] bg-white px-8 py-4 justify-between items-center shadow-[0px_5px_11px_rgba(182,194,206,0.1)] z-10 border-b border-slate-200">
            <div className="flex flex-col justify-center gap-0.5">
              <h1 className="text-[#0B424F] text-xl font-bold font-['Mona_Sans'] flex items-center gap-2">
                Pusat Pengelolaan Mitra Warung 🏪
              </h1>
              <p className="text-[#36959B] text-xs font-normal font-['Mona_Sans']">
                Validasi botol plastik warga secara instan &amp; pantau rekam transaksi realtime
              </p>
            </div>
          ...
          <main className="p-4 md:p-6 lg:p-8 flex flex-col gap-6 max-w-[1440px] w-full mx-auto font-['Mona_Sans']">
            ...
            <section className="w-full bg-white rounded-2xl p-5 md:p-6 shadow-sm border border-slate-100 flex flex-col gap-5">
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 border-b border-slate-100 pb-4">
                <div>
                  <h2 className="text-[#0B424F] text-base sm:text-lg font-bold">Riwayat Verifikasi &amp; Transaksi Warung</h2>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-fadeIn">
          <div className="w-[384px] md:w-[420px] bg-white rounded-[20px] p-6 md:p-8 flex flex-col items-center gap-5 shadow-2xl relative border border-slate-100 font-['Poppins']">
            ...
            <div className="w-full flex flex-col items-center gap-2 text-center">
              ...
              <p className="text-xs text-slate-400">Saldo aktif warung: {formatRupiah(balance)}</p>
