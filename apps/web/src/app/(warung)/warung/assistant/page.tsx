...
// Daftar pertanyaan cepat khusus Mitra Warung
const QUICK_PROMPTS = [
  "Cek saldo warung?",
  "Cara scan QR warga?",
  "Berapa komisi setoran?",
...
export default function WarungAssistantPage() {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      id: "msg-1",
      sender: "ai",
      text: "Halo Mitra Warung Bu Tejo! Ada yang bisa PANTRA bantu operasional hari ini?",
  ...
  // Handler Kirim Pesan dengan respons khusus Mitra Warung
  const handleSendMessage = (text: string) => {
    ...
    // Simulasi balasan AI untuk Warung
    setTimeout(() => {
      let aiReply = "Maaf, PANTRA belum mengerti pertanyaan tersebut.";
      
      const lowerText = text.toLowerCase();
      if (lowerText.includes("saldo")) {
        aiReply = "Saldo aktif warung saat ini adalah Rp. 12.000,00 dan siap untuk ditarik kapan saja.";
      } else if (lowerText.includes("scan") || lowerText.includes("qr")) {
        aiReply = "Untuk memindai setoran warga, silakan akses menu 'Scan QR Warga' di sidebar sebelah kiri.";
      } else if (lowerText.includes("komisi")) {
        aiReply = "Komisi mitra warung dihitung otomatis per botol yang berhasil diverifikasi ke dalam sistem.";
      } else if (lowerText.includes("riwayat")) {
        aiReply = "Anda dapat melihat daftar seluruh verifikasi dan penarikan terakhir melalui menu 'Riwayat Transaksi'.";
      }
          ...
          {isMobileMenuOpen && (
            <div className="mt-3 bg-[#0B424F] text-white p-6 rounded-[24px] flex flex-col gap-6 shadow-2xl border border-[#235D6B] animate-fadeIn">
              ...
              <nav className="flex flex-col gap-3">
                ...
                <Link href="/auth/warung/login" className="flex items-center gap-3.5 px-5 py-3.5 bg-[#235D6B] hover:bg-red-950/40 text-white border border-[#52C3BF] rounded-[16px] text-sm font-semibold font-['Poppins'] mt-2">
          ...
          <header className="hidden md:flex w-full h-[84px] bg-white px-8 py-4 justify-between items-center shadow-[0px_5px_11px_rgba(182,194,206,0.1)] z-10 border-b border-slate-200 shrink-0">
            <div className="flex flex-col justify-center gap-1">
              ...
              <p className="text-[#36959B] text-sm font-normal font-['Mona_Sans']">
                Asisten cerdas untuk pengelolaan saldo, verifikasi setoran, dan operasional warung
              </p>
            </div>
                ...
                <div className="flex items-center gap-3">
                  <div className="flex-1 relative flex items-center">
                    <textarea
                      ...
                      placeholder="Tanya seputar operasional warung..."
